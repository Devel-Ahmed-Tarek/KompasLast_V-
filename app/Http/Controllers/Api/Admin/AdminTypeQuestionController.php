...
class AdminTypeQuestionController extends Controller
{
    ...
    {
        // الحصول على جميع الترجمات لضمان وجود جميع اللغات الخمس
        $questionText = $this->ensureAllLanguages($question->getTranslations('question_text') ?: $question->question_text);
        ...
        $data = [
            ...
            'options' => $question->options->map(function ($option) {
                $optionText = $this->ensureAllLanguages($option->getTranslations('option_text') ?: $option->option_text);
                return [
                    'id' => $option->id,
        ...
        foreach ($childQuestions as $child) {
            ...
            if ($child->parent_option_id) {
                ...
                if ($parentOption) {
                    $parentOptionText = $this->ensureAllLanguages($parentOption->getTranslations('option_text') ?: $parentOption->option_text);
    ...
    /**
     * التأكد من وجود جميع اللغات الخمس في النص
     */
    private function ensureAllLanguages($translations)
    {
        ...
        // إذا لم يكن array أو JSON صحيح، نعيد array فارغ بجميع اللغات
        $result = [];
        foreach ($languages as $lang) {
            $result[$lang] = null;
        }
